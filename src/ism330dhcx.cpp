...
int QwDevISM330DHCX::getAddressLow()
{
	return ISM330DHCX_ADDRESS_LOW;
}
int QwDevISM330DHCX::getAddressHigh()
{
	return ISM330DHCX_ADDRESS_HIGH;
}
...
bool QwDevISM330DHCX::setI2C(int fd)
{
	_fd = fd;
	// sfe_dev.fd = fd;
	sfe_dev = fd;
...
int32_t QwDevISM330DHCX::writeRegisterRegion(uint8_t offset, uint8_t *data, uint16_t length)
{
	...
	// int result = wiringPiI2CWriteBlock (_fd, (int) offset, data, (int) length);
	int32_t result = ism330dhcx_write_reg(sfe_dev, offset, data, length);

	return result; // -1 = error, 0 = success
}

int QwDevISM330DHCX::readRegisterRegion(uint8_t offset, uint8_t *data, uint16_t length)
{

	// int result = wiringPiI2CReadBlock (_fd, (int) offset, data, (int) length);
	int32_t result = ism330dhcx_read_reg(sfe_dev, offset, data, length);
...
bool QwDevISM330DHCX::checkStatusAccelORGyro()
{
